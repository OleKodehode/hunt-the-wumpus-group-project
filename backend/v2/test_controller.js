...
/**
 * @typedef {import('../master_controller.js').WumpusServer} WumpusServer
 * @typedef {import('../master_controller.js').CaveMap} CaveMap
 * @typedef {import('../master_controller.js').TurnResult} TurnResult
 */
...
class WumpusClient {
  ...
  showInfo() {
    ...
    console.log(
      `Neighboring caves (possible destinations): [${neighbors.join(", ")}]`
    );
  ...
  async handleInteractiveTurn() {
    ...
    const action = (await askQuestion(`[${this.id}] Your action (M/S/P): `))
      .trim()
      .toUpperCase();
    ...
    if (action === "M") {
      try {
        const targetInput = await askQuestion(
          `[${
            this.id
          }] Where to move? (Enter cave number from [${neighbors.join(
            ", "
          )}]): `
        );
      ...
      try {
        const targetInput = await askQuestion(
          `[${
            this.id
          }] Where to shoot? (Enter cave number from [${neighbors.join(
            ", "
          )}]): `
        );
        const target = parseInt(targetInput.trim());
        ...
        if (isNaN(target) || !neighbors.includes(target)) {
          console.log(
            "Invalid target. You can only shoot into an adjacent cave number."
          );
          return false;
        }
        this.shoot(target);
...
async function gameLoop() {
  ...
  if (typeof WumpusServer === "undefined") {
    console.error(
      "WumpusServer class is required and not defined. Please ensure the server code is available."
    );
    rl.close();
    return;
  }
  ...
  const server = new WumpusServer(5, 7);
