...
export default class WumpusServer {
  ...
  constructor(minCaves = 15, maxCaves = 30) {
    ...
    this.numCaves =
      Math.floor(Math.random() * (maxCaves - minCaves + 1)) + minCaves;
  ...
  initializePlayer(playerId) {
    const safeCaves = Array.from({ length: this.numCaves }, (_, i) => i).filter(
      (c) => c !== this.wumpusLocation && !this.pits.has(c) && !this.bats.has(c)
    );
    const startCave =
      safeCaves.length > 0
        ? safeCaves[Math.floor(Math.random() * safeCaves.length)]
        : 0;
